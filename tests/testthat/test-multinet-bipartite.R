...
test_that("Bipartite N() summary with an LM and noncompacted stats", {
  summ.N <- summary(nws~N(~edges+nodematch("a"), ~1+time), term.options=list(N.compact_stats=FALSE))
...
test_that("Bipartite N() summary with offset and compacted stats", {
  summ.N <- summary(nws~N(~edges, offset=~time))
...
test_that("Bipartite Valued N() summary with an LM and noncompacted stats", {
  summ.N <- summary(nws~N(~sum+nodematch("a", form="sum"), ~1+time), term.options=list(N.compact_stats=FALSE), response="w")
...
test_that("Bipartite Valued N() summary with offset and compacted stats", {
  summ.N <- summary(nws~N(~sum, offset=~time), response="w")
