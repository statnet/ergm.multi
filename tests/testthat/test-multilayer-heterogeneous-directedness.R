...
test_that("multilayer heterogeneous layers messages", {
  ...
  nw12 %v% "mode" <- rep(2:1,c(15,5))
  nw12 %n% "nattr" <- "def"
  expect_warning(expect_warning(
    Layer(nw1, nw12, .active=list(~mode==1, ~TRUE)),
    "Vertex attribute\\(s\\) 'mode' have conflicting values in different layers\\. Earlier networks will overwrite the later\\."),
    "Network attribute\\(s\\) 'nattr' have conflicting values in different layers\\. Earlier networks will overwrite the later\\.")
})
