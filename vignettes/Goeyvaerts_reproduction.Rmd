...
We now construct the formula, which will be passed directly to `ergm()`:
```{r}
...
f.wd <- Networks(G.wd) ~
  ...
  N(
    ...
    # counts.
    #
    # Notic that since in an undirected network, tail < head, in the
    # levels2 specification, it is important that the tail levels are rows
    # and head levels are columns. In this case, "Child" comes before
    # "Father" which comes before "Mother" in alphabetical order, and so
    # which category is row= and which is col= is sorted accordingly.
  ...
  N(~triangles, ~I(n>=6))
```
See `ergmTerm?mm` for documentation on the `mm` term.
